...
all_namespaces = WATCH_NAMESPACE.split(",") if WATCH_NAMESPACE else []


def watch_namespace(namespace, **_):
    return WATCH_NAMESPACE == "" or namespace in all_namespaces


@kopf.on.create(
    "", "v1", "secrets", annotations={"synator/sync": "yes"}, when=watch_namespace
)
@kopf.on.update(
    "", "v1", "secrets", annotations={"synator/sync": "yes"}, when=watch_namespace
)
...
@kopf.on.create(
    "", "v1", "configmaps", annotations={"synator/sync": "yes"}, when=watch_namespace
)
@kopf.on.update(
    "", "v1", "configmaps", annotations={"synator/sync": "yes"}, when=watch_namespace
)
    ...
    try:
        ...
        for secret in api_response.items:
            ...
            if (
                ...
                and secret.metadata.annotations.get("synator/sync") == "yes"
            ):
                secret.metadata.annotations.pop("synator/sync")
                secret.metadata.resource_version = None
                secret.metadata.uid = None
                for ns in parse_target_namespaces(secret.metadata, [name]):
                    secret.metadata.namespace = ns
                    try:
                        api.read_namespaced_secret(secret.metadata.name, ns)
        ...
        print(
            "Exception when calling CoreV1Api->list_secret_for_all_namespaces: %s\n" % e
        )
...
def reload_pod_config(body, meta, spec, status, old, new, diff, **kwargs):
    ...
    for pod in pods.items:
        ...
        if (
            ...
            and pod.metadata.annotations.get("synator/reload")
            and any(
                "configmap:" + meta.name in s
                for s in pod.metadata.annotations.get("synator/reload").split(",")
            )
        ):
